...
import { productsData, saleProductsData } from "@/constants/dummyData";
...
const ProductDetail = (props: Props) => {
  ...
  const extendurl = productType === "sale" ? "sale/products" : "product";
  console.log("type", productType);
  const [product, setProduct] = useState<ProductType>();
  const [saleProduct, setSaleProduct] = useState<SaleItemType>();
  useEffect(() => {
    saleProductData();
  }, [id]);
  const saleProductData = () => {
    axios
      .get(`http://192.168.43.97:8000/api/v1/${extendurl}/${id}`)
      .then((res) => {
        const data = res.data;
        {
          productType === "sale" ? setProduct(data.product) : setProduct(data);
        }
        {
          productType === "sale" && setSaleProduct(data);
        }
        //  setProduct(data);
      })
      .catch((e) => {
